...
msg['From'] = sender
#邮件正文是MIMEText:
msg_content = input("{'请输入邮件主内容:'}")
msg.attach(MIMEText(msg_content, 'plain', 'utf-8'))
# 添加附件就是加上一个MIMEBase，从本地读取一个图片:
with open('./1.jpg', 'rb') as f:
    # 设置附件的MIME和文件名，这里是jpg类型,可以换png或其他类型:
    mime = MIMEBase('image', 'jpg', filename='antenna.png')
    # 加上必要的头信息:
    mime.add_header('Content-Disposition', 'attachment', filename='antenna.png')
